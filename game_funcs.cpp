...
void loadply() // 加载
{
  addanimation(18, "anime");
  {
    ...
    playerTex[9] = LoadTexture("res/main_chara/main_chara_0010.png");
  } // 玩家图片
  {
    ...
    friendliu[9] = LoadTexture("res/friend/friend_0010.png");
  } // liudoujiang
  sceneTex[1].tex = LoadTexture("res/scene/school_hall.png");
  {
    sceneTex[2].tex = LoadTexture("res/scene/class.png");
    sceneTex[2].addshade(LoadTexture("res/shade/classs_0023.png"), {207, 785},
                         {110, 689});
    sceneTex[2].addshade(LoadTexture("res/shade/classs_0022.png"), {436, 785},
                         {342, 689});
    sceneTex[2].addshade(LoadTexture("res/shade/classs_0021.png"), {680, 785},
                         {582, 690});
    sceneTex[2].addshade(LoadTexture("res/shade/classs_0020.png"), {922, 783},
                         {824, 685});
    sceneTex[2].addshade(LoadTexture("res/shade/classs_0019.png"), {1146, 788},
                         {1047, 687});
    sceneTex[2].addshade(LoadTexture("res/shade/classs_0018.png"), {1387, 788},
                         {1288, 688});
    sceneTex[2].addshade(LoadTexture("res/shade/classs_0017.png"), {207, 986},
                         {110, 887});
    sceneTex[2].addshade(LoadTexture("res/shade/classs_0016.png"), {438, 990},
                         {341, 894});
    sceneTex[2].addshade(LoadTexture("res/shade/classs_0015.png"), {687, 988},
                         {587, 891});
    sceneTex[2].addshade(LoadTexture("res/shade/classs_0014.png"), {924, 988},
                         {828, 891});
    sceneTex[2].addshade(LoadTexture("res/shade/classs_0013.png"), {1152, 993},
                         {1055, 891});
    sceneTex[2].addshade(LoadTexture("res/shade/classs_0012.png"), {1393, 1001},
                         {1295, 903});
    sceneTex[2].addshade(LoadTexture("res/shade/classs_0011.png"), {207, 1154},
                         {110, 1054});
    sceneTex[2].addshade(LoadTexture("res/shade/classs_0010.png"), {439, 1151},
                         {342, 1054});
    sceneTex[2].addshade(LoadTexture("res/shade/classs_0009.png"), {686, 1151},
                         {587, 1054});
    sceneTex[2].addshade(LoadTexture("res/shade/classs_0008.png"), {938, 1146},
                         {838, 1052});
    sceneTex[2].addshade(LoadTexture("res/shade/classs_0007.png"), {1174, 1143},
                         {1076, 1040});
    sceneTex[2].addshade(LoadTexture("res/shade/classs_0006.png"), {1473, 1167},
                         {1372, 1072});
    sceneTex[2].addshade(LoadTexture("res/shade/classs_0001.png"), {958, 621},
                         {769, 458});
    sceneTex[2].addshade(LoadTexture("res/shade/classs_0002.png"), {1649, 990},
                         {1597, 897});
    sceneTex[2].addshade(LoadTexture("res/shade/classs_0003.png"), {1644, 818},
                         {1590, 723});
    sceneTex[2].addshade(LoadTexture("res/shade/classs_0005.png"), {1626, 650},
                         {1574, 558});
  ...
  sounds["save"]=LoadSound("res/sounds/save.wav");
  {
    ...
    sceneTex[3].addshade(LoadTexture("res/shade/outdoorchair.png"), {344, 810},
                         {253, 763});
    sceneTex[3].addshade(LoadTexture("res/shade/outdoortree.png"), {1900, 842},
                         {1294, 233});
    sceneTex[3].addshade(LoadTexture("res/shade/outdoorshel.png"), {392, 887},
                         {174, 544});
  } // scene outdoor1
...
void playdia(std::vector<dia> dial, Font font, int where) { // 显示对话
  ...
  if (g_game.dialpt[where] < dial.size()) {
    ...
    DrawTextEx(font, "按下Q继续", {BASE_GAME_W - 200.0f, BASE_GAME_H - 100.0f},
               20, 1, WHITE);
    if (!dial[g_game.dialpt[where]].who.empty()) {
      DrawTextEx(font, (dial[g_game.dialpt[where]].who + ":").c_str(),
                 {20, (float)2 * BASE_GAME_H / 3 + 20}, 48, 1,
...
void movply(Player &p) // 移动玩家
{
  ...
  DrawEllipse((int)p.pos.x + playerW / 2, (int)p.pos.y + playerH,
              playerW * 0.4f, 10.0f, (Color){0, 0, 0, 80});
...
void showply(Player p) {
  ...
  Rectangle dstRect = {p.pos.x, p.pos.y, curTex.width * spriteScale,
...
void tstcordobj(Vector2 &pos, float xb1, float xb2, float yb1, float yb2, float wid, float hi) // 设置物品碰撞
{
  ...
  if (overlapX < overlapY) {
    ...
    pos.y += (playerFeetTop + feetHeight * 0.5f < (yb1 + yb2) * 0.5f)
                 ? -overlapY
                 : overlapY;
  }
}
...
void drawani(PointAnim &anim, Vector2 target, std::string which) // 绘画指针
{
  ...
  anim.nowPos =
      (cycle < 1.0f) ? Vector2{target.x, target.y + pointerShift} : target;
...
void readwhere() {
  ...
  if (!file.is_open()) {
    std::cerr << "Warning: No save file found at res/where.txt, using default "
                 "spawn position\n";
    // 使用默认出生点
  ...
  if (file >> where >> pos.x >> pos.y) {
    ...
    std::cout << "Save loaded: scene=" << where << " pos=(" << pos.x << ","
              << pos.y << ")\n";
  } else {
